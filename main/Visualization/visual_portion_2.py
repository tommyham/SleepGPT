...
# 读取所有被试的混淆矩阵，并计算指标，返回每个被试的分布值
def get_metrics_per_subject(res_or_path, subject, modes):
    """
    读取 res.json（或已加载的 dict），结构：
        {
          "<mode>": {
            "<subject>": {
              "<test_id_1>": [[...5x5...]],
    ...
    返回：
      metrics_results: {'acc': [orig_mean, aug_mean, ...],
                        'mf1': [...],
                        'kappa': [...],
                        'f1_per_class': [np.array(5,), np.array(5,), ...] }
      individual_values: {'acc': [[test_acc...], [test_acc...], ...],
                          'mf1': [[...], ...],
                          'kappa': [[...], ...],
                          'f1_per_class': [np.array(n_test,5), np.array(n_test,5), ...]}
    其中外层列表顺序与传入的 modes 一一对应。
    """
    # 允许传入文件路径或已加载的 dict
    if isinstance(res_or_path, (str, os.PathLike)):
        with open(res_or_path, "r") as f:
            res = json.load(f)
    else:
        res = res_or_path

    metrics_results = {'acc': [], 'mf1': [], 'kappa': [], 'f1_per_class': []}
    individual_values = {'acc': [], 'mf1': [], 'kappa': [], 'f1_per_class': []}

    # 累加总体混淆矩阵（如果后续需要用）
    overall_cm = np.zeros((5, 5), dtype=np.float64)

    for mode in modes:
        # 兼容：某个 mode 或 subject 可能不存在
        mode_dict = res.get(mode, {})
        subj_dict = mode_dict.get(subject, {})

        acc_vals, mf1_vals, kappa_vals, f1_per_class_vals = [], [], [], []

        for _test_id, cm in subj_dict.items():
            cm = np.array(cm, dtype=np.float64)
            overall_cm += cm

            acc, mf1, kappa, f1_per_class = compute_metrics(cm)  # 你已有的函数
            acc_vals.append(acc)
            mf1_vals.append(mf1)
            kappa_vals.append(kappa)
            f1_per_class_vals.append(f1_per_class)
        ...
        # 把当前 mode 的统计推进结果
        if len(acc_vals) == 0:
            # 若当前 mode 没有该 subject 的数据，则填充 NaN/空，保证长度对齐
            metrics_results['acc'].append(np.nan)
            metrics_results['mf1'].append(np.nan)
            metrics_results['kappa'].append(np.nan)
            metrics_results['f1_per_class'].append(np.full(5, np.nan))

            individual_values['acc'].append([])
            individual_values['mf1'].append([])
            individual_values['kappa'].append([])
            individual_values['f1_per_class'].append(np.empty((0, 5)))
        else:
            f1_per_class_vals = np.asarray(f1_per_class_vals, dtype=np.float64)  # (n_test, 5)

            metrics_results['acc'].append(float(np.mean(acc_vals)))
            metrics_results['mf1'].append(float(np.mean(mf1_vals)))
            metrics_results['kappa'].append(float(np.mean(kappa_vals)))
            metrics_results['f1_per_class'].append(np.mean(f1_per_class_vals, axis=0))
    ...
    return res, metrics_results, individual_values
...
def plot_per_metrics(metrics_per_subject, individual_values, modes, metric, start=0, end=1, name='edf'):
    ...
    for i, subject in enumerate(subjects):
        print(subject, metrics_per_subject[metric][1][i])
        # 原始和增强的对比，绘制并排的柱状图
    ...
    # 保存figure并显示
    plt.legend()
    plt.savefig(f'/Users/hwx_admin/Sleep/result/aug_portion_mass_res/portion_mass_bar/{metric}_{name}.svg')
    plt.show()
...
def _compute_metrics_from_cm(cm: np.ndarray):
    """从整数混淆矩阵计算 acc / macro-f1 / kappa / per-class f1"""
    ...
    return float(acc), float(mf1), float(kappa), f1pc.astype(float)

def _load_resjson(resjson, subject_names, modes):
    """
    统一把输入转成 {mode: {subject: {run_key: confusion_matrix}}} 结构的 dict。
    支持：
      - resjson 为 dict（已是目标结构）
      - resjson 为目录：读取 {mode}_{subject}.json
      - resjson 为文件：整体 json，结构为 {mode: {subject: {...}}}
    """
...
def save_raw_runs(
        ...
        modes=('orig', 'aug'),
        out_dir="/Users/hwx_admin/Sleep/result/aug_portion_mass_res/portion_mass_bar",
        filename="ten_runs_rawdata.xlsx"
):
    """
    读取嵌套结构 resjson（或路径），逐个混淆矩阵计算 acc / mf1 / kappa / per-class F1，
    并保存“每次实验的 raw data”。

    输出 (Excel)：
      - Sheet 'acc'        : subject, mode, run, value
      - Sheet 'mf1'        : subject, mode, run, value
      - Sheet 'kappa'      : subject, mode, run, value
      - Sheet 'perclass_f1': subject, mode, run, class, f1
    """
    os.makedirs(out_dir, exist_ok=True)
    xlsx_path = os.path.join(out_dir, filename)

    # 统一加载为 {mode: {subject: {run_key: cm}}}
    res = _load_resjson(resjson, subject_names, modes)

    acc_rows, mf1_rows, kappa_rows, pc_rows = [], [], [], []
    ...
    for mode in modes:
        ...
        for subject in subject_names:
            run_dict = mode_dict.get(subject, {})
            if not isinstance(run_dict, dict) or len(run_dict) == 0:
                print(f"[WARN] {mode}/{subject} 无可用结果，跳过。")
                continue

            # 稳定顺序
            run_keys = sorted(run_dict.keys(), key=lambda x: str(x))

            for idx, rk in enumerate(run_keys, start=1):
                cm = np.asarray(run_dict[rk], dtype=int)
                acc, mf1, kappa, f1_arr = _compute_metrics_from_cm(cm)

                acc_rows.append({"subject": subject, "mode": mode, "run": idx, "value": acc})
                mf1_rows.append({"subject": subject, "mode": mode, "run": idx, "value": mf1})
                kappa_rows.append({"subject": subject, "mode": mode, "run": idx, "value": kappa})

                n_cls = f1_arr.size
                if n_cls == 5:
                    class_labels = ["W", "N1", "N2", "N3", "REM"]
                else:
                    class_labels = [f"Class{i}" for i in range(n_cls)]

                for ci, lab in enumerate(class_labels):
                    pc_rows.append({
                        "subject": subject, "mode": mode, "run": idx,
                        "class": lab, "f1": float(f1_arr[ci])
                    })

    # 写 Excel（若 openpyxl 不可用，退回 CSV）
    try:
        with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
            pd.DataFrame(acc_rows).to_excel(writer, sheet_name="acc", index=False)
            pd.DataFrame(mf1_rows).to_excel(writer, sheet_name="mf1", index=False)
            pd.DataFrame(kappa_rows).to_excel(writer, sheet_name="kappa", index=False)
            pd.DataFrame(pc_rows).to_excel(writer, sheet_name="perclass_f1", index=False)
        print(f"✅ raw data 已保存到：{xlsx_path}")
    except Exception as e:
        print(f"[WARN] 写入 Excel 失败（{e}），改为导出 CSV。")
        pd.DataFrame(acc_rows).to_csv(os.path.join(out_dir, "acc.csv"), index=False)
        pd.DataFrame(mf1_rows).to_csv(os.path.join(out_dir, "mf1.csv"), index=False)
        pd.DataFrame(kappa_rows).to_csv(os.path.join(out_dir, "kappa.csv"), index=False)
        pd.DataFrame(pc_rows).to_csv(os.path.join(out_dir, "perclass_f1.csv"), index=False)
        print(f"✅ raw data 已保存为 CSV 于：{out_dir}")
...
if __name__ == "__main__":
    ...
    for i, subject in enumerate(subject_names):
        res_json, metrics_per_subject, individual_values = get_metrics_per_subject(data_path, subject, modes)
